...
const config = {
  ...
  tokenKey: 'smartsport/token', // 保存当前登陆用户的token，获得通过localStorage.getItem(config.tokenKey)
  userInfoKey: 'smartsport/user', // 保存当前登陆用户的用户信息，获得通过localStorage.getItem(config.userInfoKey)
};
// 环境
const environment = {
  development: {
